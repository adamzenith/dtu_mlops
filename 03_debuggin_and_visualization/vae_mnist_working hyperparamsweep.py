...
class Encoder(nn.Module):
    def __init__(self, input_dim, hidden_dim,hidden_dim2, latent_dim):
        ...
        self.FC_mean = nn.Linear(hidden_dim, hidden_dim2)
        self.FC_mean2 = nn.Linear(hidden_dim2, latent_dim)
        self.FC_var = nn.Linear(hidden_dim, hidden_dim2)
        self.FC_var2 = nn.Linear(hidden_dim2, latent_dim)
        self.training = True

    def forward(self, x):
        h_ = torch.relu(self.FC_input(x))
        mean = torch.relu(self.FC_mean(h_))
        mean = torch.relu(self.FC_mean2(mean))
...
class Decoder(nn.Module):
    def __init__(self, latent_dim, hidden_dim,hidden_dim2, output_dim):
        ...
        self.FC_hidden2 = nn.Linear(hidden_dim,hidden_dim2)
        self.FC_output = nn.Linear(hidden_dim2, output_dim)
    ...
    for batch_idx, (x, _) in enumerate(train_loader):
        ...
        if batch_idx%50==0:
            with torch.no_grad():
                ...
                wandb.log({"Generated Image:": wandb.Image(generated_image.view(32, 1, 28, 28))})
        optimizer.zero_grad()
